...
import { deleteParticipantOfGroup, getAllGroups, getJoinedGroupsByUser, getUser, joinNewGroupByUser } from '../../services/api';
...
function Groups() {
    ...
    function handleAlreadyJoinedGroupOrNot(joinedList, groupUserWantJoin) {
        ...
        if (joinedList.AdminsGroup.length === 0)
            return false;
    ...
    function handleJoinNewGroup(userId, groupId) {
        ...
        promise.catch(error => {
            if (error.response.status === 409)
                alert("voce ja participa desse grupo")
            else {
    ...
    return (
        <Container>
            ...
            <MyGroups handleJoinClick={removeMyGroup} groupsUserJoined={groupsUserJoined} />
        </Container>
    );
}
